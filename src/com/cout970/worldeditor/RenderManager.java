...
//		GLManager.camara.renderCamara();
	...
	public static void renderBlock(Block bl){
		glPushMatrix();
		glTranslatef((float)bl.getX(), (float)bl.getY(),(float)bl.getZ());
		if(WorldEditor.getSelectedBlock() == bl && ToolBox.action == Action.Select)
			RenderUtil.bindTexture(TextureManager.select);
		else
			RenderUtil.bindTexture(TextureManager.getTexture(bl.material));
		//glColor3f(b.material.colorMaterial[0], b.material.colorMaterial[1], b.material.colorMaterial[2]);
		float r= 1 * bl.material.colorMaterial[0];
		float g= Math.max(0, 1-(float)bl.calor/1400) * bl.material.colorMaterial[1];
		float b= Math.max(0, 1-(float)bl.calor/900) * bl.material.colorMaterial[2];
		glColor3f(r, g, b);
		RenderUtil.block(bl);
		glEnd();
		glPopMatrix();
